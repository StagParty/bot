...
class Applications(commands.Cog):
    ...
    async def apply(self, ctx):
        ...
        await ctx.reply(
            f"An applcation channel for you has been created! {channel.mention}"
        )

        e = discord.Embed(
            title="Staff Application",
            description="This is StagParty's Interactive Staff Application. I will be interviewing you with a series of questions. When answering, please keep your replies in one message only.",
            color=THEME,
        )
        ...
        for ques in questions:
            ...
            try:
                prev_answer_msg = await self.bot.wait_for(
                    "message", timeout=300, check=check
                )
                answers.append(prev_answer_msg.content)
            except asyncio.TimeoutError:
                await channel.send(
                    "This application has been inactive for a while, closing."
                )
        ...
        embed = discord.Embed(
            ...
            description=f"User information:\nName: {ctx.author.name}\nID: {ctx.author.id}\nJoined: <t:{int(ctx.author.joined_at.timestamp())}>",
        ...
        await channel.send(
            "Your application has been submitted! This channel will be deleted in 5 seconds."
        )
    ...
    async def accept(self, ctx, id: int):
        if ctx.author.id in OWNERS:
            ...
            e = discord.Embed(
                ...
                description=f"Your application has been **ACCEPTED**!\n\nWelcome to the staff team! We'll give you a 7-day trial period. ",
